...
$con = getDBConnection(); // Get the database connection
$stmt = $con->prepare("SELECT lawyer_first_name, lawyer_last_name, lawyer_email, lawyer_phone_no, lawyer_city, lawyer_address, lawyer_rating, specialization from lawyer_login WHERE lawyer_id = ?");
$stmt->bind_param('s', $_SESSION['lawyer_id']);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($fname, $lname, $email, $phone, $city, $address, $rating, $special);
$stmt->fetch();
